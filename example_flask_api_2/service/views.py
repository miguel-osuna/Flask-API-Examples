...
class UserListResource(Resource):
    ...
    def post(self):
        ...
        if existing_user is not None:
            response = {
                "user": "An user with the name {} already exists".format(user_name)
            }
            return response, HttpStatus.bad_request_400.value
...
class NotificationCategoryListResource(AuthenticationRequiredResource):
    ...
    def post(self):
        ...
        if not NotificationCategory.is_name_unique(
            ...
            response = {
                "error": "A notification category with the name {} already exists".format(
                    notification_category_name
                )
            }
...
service.add_resource(UserListResource, "/users/")
